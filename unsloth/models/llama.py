...
from transformers.models.llama.modeling_llama import BaseModelOutputWithPast, CausalLMOutputWithPast, logger
...
# For Pytorch 2.1.1
try:
    from transformers.models.llama.modeling_llama import LlamaFlashAttention2, LlamaSdpaAttention
except:
...
from peft.config import from_pretrained as peft_config_from_pretrained
from peft.peft_model import from_pretrained as peft_model_from_pretrained
...
def LlamaAttention_fast_forward_inference(
    ...
    if do_prefill:
        self.paged_attention = torch.empty(
            (KV_CACHE_INCREMENT + seq_len + 1, 2, bsz, n_kv_heads, head_dim), dtype=dtype, device='cuda:0'
        )
    ...
    if not HAS_FLASH_ATTENTION and HAS_XFORMERS and attention_mask is None:
        ...
        if SDPA_HAS_GQA:
            ...
            A = scaled_dot_product_attention(
                Q, K, V, attn_mask=attention_mask, is_causal=False, enable_gqa=n_groups != 1
            )
    ...
    **kwargs,
    ...
    if False:  # (past_key_values_length != 0):
        position_ids = torch.arange(
            past_key_values_length, seq_length + past_key_values_length, dtype=torch.int32, device='cuda:0'
        )
    ...
    if IS_GEMMA2:
        if HAS_FLASH_ATTENTION_SOFTCAPPING and attention_mask is None:
            ...
            if HAS_FLEX_ATTENTION:
                ...
                self.SWA_mask = (
                    AttentionMaskConverter(is_causal=True, sliding_window=self.config.sliding_window)
                ...
                self.GA_mask = (
                    AttentionMaskConverter(is_causal=True)
                    .to_causal_4d(1, n, n, dtype=inputs_embeds.dtype, device='cuda:0')
                    .squeeze(0)
                    .squeeze(0)
                )
    ...
    if (
        IS_ATTENTION_REFACTOR and (hasattr(self, 'rotary_emb') or not hasattr(self.layers[0].self_attn, 'rotary_emb'))
    ) or IS_GRANITE:
...
# https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L825
def LlamaModel_fast_forward_inference(self, input_ids, past_key_values, position_ids, attention_mask=None):
    input_ids = input_ids[:, : self.max_seq_length]
    ...
    if bsz != 1:
        attention_mask = _prepare_4d_causal_attention_mask_for_sdpa(
            attention_mask, (bsz, q_len), X, seq_len, sliding_window=getattr(self.config, 'sliding_window', None)
        )
    else:
        attention_mask = None
    pass
    ...
    for idx, decoder_layer in enumerate(self.model.layers):
        ...
        X = fast_rms_layernorm_inference(decoder_layer.input_layernorm, X, XX=XX, XX2=XX2, variance=variance)
        ...
        residual.copy_(X)  # residual = X
        X = fast_rms_layernorm_inference(decoder_layer.post_attention_layernorm, X, XX=XX, XX2=XX2, variance=variance)
        X = fast_swiglu_inference(decoder_layer.mlp, X, temp_gate=temp_gate, temp_up=temp_up)
        X += residual

        next_decoder_cache.append(present_key_value)
    pass
    X = fast_rms_layernorm_inference(self.model.norm, X, XX=XX, XX2=XX2, variance=variance)

    return BaseModelOutputWithPast(
        last_hidden_state=X, past_key_values=next_decoder_cache, hidden_states=[], attentions=[]
    )
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        self.register_buffer(
            'cos_cached', emb.cos().to(dtype=dtype, device=device, non_blocking=True), persistent=False
        )
        self.register_buffer(
            'sin_cached', emb.sin().to(dtype=dtype, device=device, non_blocking=True), persistent=False
        )
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def __init__(
        ...
        super().__init__(
            dim=dim, max_position_embeddings=max_position_embeddings, base=base, device=device, config=config
        )
...
class LongRopeRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(
        ...
        t = torch.arange(
            original_max_position_embeddings, device=self.short_inv_freq.device, dtype=torch.int64
        ).float()
    ...
    def forward(self, x, position_ids=None, seq_len=None):
        ...
        if seq_len < self.original_max_position_embeddings:
            ...
            return (self.long_cos_cached[:seq_len].to(dtype=x.dtype), self.long_sin_cached[:seq_len].to(dtype=x.dtype))
        pass
...
>>>>>>> c261c0d (modidyinf loader and llama to load lora)
        ...
        if original_llm_model is not None:
            model_config = AutoConfig.from_pretrained(original_llm_model, token=token, attn_implementation='sdpa')
        else:
            model_config = AutoConfig.from_pretrained(model_name, token=token, attn_implementation='sdpa')
        ...
        if original_llm_model is not None:
            ...
            peft_config = peft_config_from_pretrained(original_llm_model)
            model = peft_model_from_pretrained(model_, peft_config)
        ...
        inner_training_loop = inner_training_loop.replace(
            'train_dataloader = self.get_train_dataloader()', check_batches, 1
        )
        inner_training_loop = inner_training_loop.replace('_inner_training_loop', '_fast_inner_training_loop', 1)
        ...
        if 'n_total_devices >' not in inner_training_loop:
            raise RuntimeError('Unsloth currently does not support multi GPU setups - but we are working on it!')
        pass
        inner_training_loop = inner_training_loop.replace('is_sagemaker_mp_enabled()', 'False')
        exec(inner_training_loop, globals())
        Trainer._inner_training_loop = _fast_inner_training_loop
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if type(r) is not int:
            raise TypeError(f'Unsloth: Rank of {r!s} must be an integer.')
        if r <= 0:
            raise TypeError(f'Unsloth: Rank of {r!s} must be larger than 0.')
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            # Combine all
            new_target_modules = list(target_modules) + list(modules_to_save if modules_to_save is not None else [])
            ...
            if check_all:
                ...
                if 'embed_tokens' in new_target_modules:
                    ...
                    model.get_input_embeddings().original_module.to(device='cpu', non_blocking=True)
                ...
                if 'lm_head' in new_target_modules:
                    ...
                    model.get_output_embeddings().original_module.to(device='cpu', non_blocking=True)
                    model.get_output_embeddings().original_module.requires_grad_(False)
                pass

                return model
            else:
                raise TypeError('Unsloth: Your model already has LoRA adapters. Your new parameters are different.')
            pass
        pass
        ...
        if not (type(init_lora_weights) is bool or init_lora_weights == 'gaussian' or init_lora_weights == 'loftq'):
            raise ValueError('Unsloth: `init_lora_weights` must be either [True, False, "gaussian", "loftq"].')
        pass
        ...
        accepted_modules = frozenset(('q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'))
        model.config.update({'unsloth_version': __version__})
    ...
    def patch_peft_model(model, use_gradient_checkpointing=True):
        ...
        model = prepare_model_for_kbit_training(
            model, use_gradient_checkpointing=use_gradient_checkpointing, use_reentrant=True
        )
        ...
        active_adapter = model.active_adapters[0] if hasattr(model, 'active_adapters') else model.active_adapter
        ...
        _apply_lora_mlp = partial(apply_lora_mlp, inplace=False) if model_type == 'cohere' else apply_lora_mlp
        pass
