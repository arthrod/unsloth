...
from ._utils import HAS_FLASH_ATTENTION, HAS_FLASH_ATTENTION_SOFTCAPPING, is_bfloat16_supported
        ...
        **kwargs,
        ...
        try:
            model_config = AutoConfig.from_pretrained(
                model_name, token=token, revision=revision, trust_remote_code=trust_remote_code
            )
            is_model = True
        ...
        try:
            if original_llm_model is not None:
                peft_config = PeftConfig.from_pretrained(
                    original_llm_model, token=token, revision=revision, trust_remote_code=trust_remote_code
                )
            else:
                peft_config = PeftConfig.from_pretrained(
                    model_name, token=token, revision=revision, trust_remote_code=trust_remote_code
        ...
        if fast_inference:
            from unsloth_zoo.vllm_utils import patch_vllm, vllm_dynamic_quant_supported
...
from ..kernels import patch_loss_functions
from ._utils import (
